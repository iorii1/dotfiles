...
hl.gesture({ fingers = 2, direction = "left",  action = function() hl.dispatch(hl.dsp.focus({ direction = "left" }))  end })
hl.gesture({ fingers = 2, direction = "right", action = function() hl.dispatch(hl.dsp.focus({ direction = "right" })) end })
hl.gesture({ fingers = 2, direction = "up",    action = function() hl.dispatch(hl.dsp.focus({ direction = "up" }))    end })
hl.gesture({ fingers = 2, direction = "down",  action = function() hl.dispatch(hl.dsp.focus({ direction = "down" }))  end })
...
hl.bind("XF86AudioRaiseVolume",  hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_SINK@ 5%+ && qs ipc call osd volume"),    { locked = true })
hl.bind("XF86AudioLowerVolume",  hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_SINK@ 5%- && qs ipc call osd volume"),    { locked = true })
hl.bind("XF86AudioMute",         hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_SINK@ toggle && qs ipc call osd volume"),   { locked = true })
...
hl.bind("SHIFT + XF86MonBrightnessUp",   hl.dsp.exec_cmd("brightnessctl s 100% && qs ipc call osd brightness"), { locked = true })
hl.bind("XF86MonBrightnessDown",         hl.dsp.exec_cmd("brightnessctl s 2%- && qs ipc call osd brightness"),  { locked = true })
hl.bind("SHIFT + XF86MonBrightnessDown", hl.dsp.exec_cmd("brightnessctl s 1% && qs ipc call osd brightness"),   { locked = true })
